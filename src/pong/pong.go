...
func (ball *ball) update(leftPaddle *paddle, rightPaddle *paddle) {
	...
	if int(ball.x) < int(leftPaddle.pos.x)+leftPaddle.w/2 {
...
// Play will start playing pong
func Play() {
	sdl.Init(sdl.INIT_EVERYTHING)
	defer sdl.Quit()
	window, err := sdl.CreateWindow("Pong", sdl.WINDOWPOS_UNDEFINED, sdl.WINDOWPOS_UNDEFINED, 800, 600, sdl.WINDOW_SHOWN)
	...
	player2 := paddle{pos{float32(winWidth - 50), 300}, 20, 100, color{255, 255, 255}}
